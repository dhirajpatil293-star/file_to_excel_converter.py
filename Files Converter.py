...
# Configure Tesseract path if running on Windows locally
...
# Page Configuration
st.set_page_config(
    page_title="File to Excel Converter", page_icon="⚡", layout="wide"
)

st.title("⚡ File-to-Excel Converter Web App")
st.write(
    "Upload a **PDF**, **TXT**, **CSV**, or **Image (JPG/PNG)** file to parse its content and export it directly as an Excel spreadsheet."
)
            ...
            for line in stringio:
                ...
                if line_str:
                    ...
                    if "," in line_str:
                        ...
                        parsed_rows.append(
                            [t.strip() for t in line_str.split("\t")]
                        )
...
uploaded_file = st.file_uploader(
    "Choose a file to convert", type=["pdf", "txt", "csv", "jpg", "jpeg", "png"]
)
...
if uploaded_file is not None:
    ...
    if not parsed_rows and not raw_lines:
        ...
        # Detect default suggested headers
        ...
        headers_input = st.text_input(
            "Enter column header names separated by commas:",
            value=default_headers_str,
        )

        custom_headers = [
            h.strip() for h in headers_input.split(",") if h.strip()
        ]
        ...
        if custom_headers:
            ...
            if parsed_rows:
                ...
                if any(
                    "part" in str(cell).lower() for cell in source_rows[0]
                ):
                ...
                if any(
                    k in source_lines[0].lower()
                    for k in ["part", "item", "component"]
                ):
                ...
                for line in source_lines:
                    ...
                    for idx, header in enumerate(custom_headers):
                        row_dict[header] = (
                            tokens[idx] if idx < len(tokens) else ""
                        )
                    formatted_rows.append(row_dict)
            ...
            with pd.ExcelWriter(output_buffer, engine="openpyxl") as writer:
                ...
                header_fill = PatternFill(
                    start_color="00472B", end_color="00472B", fill_type="solid"
                )
                header_font = Font(
